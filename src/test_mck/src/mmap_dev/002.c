	...
	unsigned long size)
{
	...
	/* mmap */
	data = (char*)mmap((caddr_t)0, size, PROT_READ|PROT_WRITE, MAP_SHARED, fd, 0);
	printf("mmap()=%p\n", data);
	...
	/* validate */
	peek_data = (char*)mmap((caddr_t)0, size, PROT_READ|PROT_WRITE, MAP_SHARED, fd, 0);
	printf("peek mmap()=%p\n", peek_data);
	tp_assert(peek_data != (char*)-1, "peek mmap error.");
	val = 1;
	for (i = 0; i < pages; i++) {
		int cmp;
		char *ptr = peek_data + PAGE_SIZE * i;
	...
	status = munmap(peek_data, size);
	printf("munmap()=%d\n", status);
	tp_assert(status != -1, "peek munmap error.");
