/* 007.c COPYRIGHT FUJITSU LIMITED 2015-2016 */
#include "test_mck.h"
#include "testsuite.h"
#include <sys/mman.h>
...
RUN_FUNC(TEST_SUITE, TEST_NUMBER)
{
	...
	const int old_prot = PROT_WRITE|PROT_READ;
	const int new_prot = PROT_WRITE|PROT_READ|PROT_EXEC;
	char *p;
	int ret;

	p = mmap(NULL, size, old_prot, flags, -1, 0);
	printf("mapped=%p\n", p);
	tp_assert(p != (void*)-1UL, "out of memory.(mmap)");
