/* 013.c COPYRIGHT FUJITSU LIMITED 2016 */
/* ptrace(PTRACE_SETSIGINFO) testcase */
#include "test_mck.h"
#include <semaphore.h>
#include <signal.h>
#include <unistd.h>
...
static int parent_func(sem_t *swait, sem_t *swake, pid_t cpid)
{
	...
	/* read child siginfo */
	if (ptrace(PTRACE_GETSIGINFO, cpid, 0, &org_info)) {
		perror("ptrace(PTRACE_GETSIGINFO)");
		goto cont;
	}

	/* make write siginfo */
	write_info.si_signo = org_info.si_signo + 1;
	write_info.si_errno = org_info.si_errno + 1;
	write_info.si_code = org_info.si_code + 1;

	/* write child siginfo */
	if (ptrace(PTRACE_SETSIGINFO, cpid, 0, &write_info)) {
		perror("ptrace(PTRACE_SETSIGINFO)");
		goto cont;
	}

	/* read child siginfo */
	if (ptrace(PTRACE_GETSIGINFO, cpid, 0, &tmp_info)) {
		perror("ptrace(PTRACE_GETSIGINFO)");
		goto cont;
	}

	/* check */
	if (write_info.si_signo != tmp_info.si_signo) {
		printf("si_signo not change.\n");
		goto cont;
	}

	if (write_info.si_errno != tmp_info.si_errno) {
		printf("si_errno not change.\n");
		goto cont;
	}

	if (write_info.si_code != tmp_info.si_code) {
		printf("si_code not change.\n");
		goto cont;
	}

	printf("siginfo change value check OK.\n");

	/* restore */
	if (ptrace(PTRACE_SETSIGINFO, cpid, 0, &org_info)) {
		perror("ptrace(PTRACE_SETSIGINFO)");
		goto cont;
	}
